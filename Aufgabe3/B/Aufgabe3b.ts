document.addEventListener('DOMContentLoaded', function () {
    ...
    var filedCards: undefined = [];
    document.getElementById("Draw").addEventListener("click", function () {
        if (HandCards.length < 5 && cards.length > 0) {
            var cardsnummer: number = Math.floor((Math.random() * 31) + 0); // zufällige Zahl (zwischen 0 und 31)
            ...
            var div: HTMLDivElement = document.createElement("div"); // Darstellung der Karte auf der Hand
