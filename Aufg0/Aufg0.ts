namespace L4_Skipiste {
    ...
    let crc2: CanvasRenderingContext2D;
     function init(_event: Event): void {
        ...
        let colors = [];
        for (let i = 0; i < 15; i++) {
            let randomX = (Math.random() * (800 - 1)) + 1; //bestimmen, wo auf der x-Länge Bäume sind
            let randomY = (Math.random() * (550 - 400)) + 400; //bestimmen, wo auf der y-Länge Bäume sind
            console.log("X is " + randomX, "Y is " + randomY, randomTree1);
            let randomTree1 = Math.floor((Math.random() * 2)) + 1;
            if (randomTree1 == 1) {
